      ...
      fs.exists(convertedFilePath(uploadPath))
        .then((exist) => {
          ...
          if (exist) {
            ...
            log.info(
              `Could not find media-object at path ${uploadPath} stored in server message ${this.props.currentMessage._id}. Displaying record Button again.`,
            );
            this.mediaPath = '';
            this.setState({
              submitted: false,
        ...
        CommonUtils.cacheLocalMedia(
          ...
          .then((filepath) => {
            ...
            CommonUtils.deleteLocalFile(this.mediaPath)
              ...
              .catch((e) =>
                log.warn(
                  'Error while trying to remove temporary file: ' +
                    e.toString(),
                ),
              );
            log.info(
