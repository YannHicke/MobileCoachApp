...
export function* initializeServerSync(action) {
  ...
  const unsubscribe = NetInfo.addEventListener((state) => {
    // TODO fabian: properly unsubscribe also check for internet connection using isInternetReachable. Hence check what was the old APIs behaviour in react-native/netinfo
...
export function* watchOutgoingMessageChannel() {
  ...
  while (true) {
    ...
    if (
      ...
      log.debug(
        'The current role cannot send such a message, so skip this one',
      );
      continue;
    }
    ...
    switch (message.type) {
      case MessageTypes.PLAIN:
        ...
        if (message['related-message-id'] !== undefined) {
          messageObject['related-message-id'] =
            message['related-message-id'].substring(2);
        }
...
function* doSynchronization(settings) {
  ...
  const listenerRegistrationSuccessful = yield call(
    registerListeners,
    settings,
  );
...
async function sendPreparedMessages(messageKeys, messageObjects, userMessages) {
  if (userMessages) {
