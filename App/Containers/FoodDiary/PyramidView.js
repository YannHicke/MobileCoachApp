...
} from '../AddMealModule/FoodMetrics'
...
class PyramidView extends Component {
  constructor (props) {
    ...
    trackingPeriods.forEach((trackingPeriod, i) => {
      ...
      if (completed) {
        this.data.push({
          value: '' + i,
          label: I18n.t('FoodDiary.trackingPeriodNumber') + ' ' + (i + 1)
        })
      }
    })
  ...
  render () {
    ...
    if (
      foodDiary.trackingPeriods[0] &&
      this.props.foodDiary.trackingPeriods[0].completed
    ) {
      ...
      return (
        <View style={styles.container}>
          <Text style={styles.missingDataText}>
            {I18n.t('FoodDiary.trackingPeriodIncompleteNotice')}
          </Text>
        </View>
      )
    }
  ...
  renderContent () {
    return (
      <ScrollView style={styles.container}>
        <View style={styles.selectionBar}>{this.renderDropDown()}</View>
    ...
    return (
      <Dropdown
        ...
        onChangeText={(value, index, data) =>
          this.onSelectionChanged(value, index, data)
        }
      />
    )
  }
  ...
  renderPyramidContainer () {
    ...
    let includedDays = [
      ...trackingPeriods[selectedPeriod].trackedDaysComplete
    ].reverse()
    ...
    return (
      <View>
        ...
        <Text style={styles.includingDaysTitle}>
          {I18n.t('FoodDiary.trackingPeriodNumber') +
            ' ' +
...
class Pyramid extends Component {
  ...
  measureView (event) {
    ...
    this.setState({
      pyramidWidth:
        this.props.pyramidScale * event.nativeEvent.layout.width -
        horizontalPadding * 2,
      pyramidContainerWidth:
        event.nativeEvent.layout.width - horizontalPadding * 2
    })
  }
...
class PyramidBar extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (!isNaN(this.percentage)) {
      ...
      Animated.timing(this.state.percentageAnimation, {
        toValue: newPosition,
        duration: 500
      }).start()
    }
  ...
  render () {
    ...
    let scaledPositionTop =
      (position - 1) * (barHeight + barMarginHeight) +
      (scaleFactor * BASE_DIMENSIONS.topHeight + barMarginHeight)
    ...
    let scaledRightPosition =
      pyramidContainerWidth - pyramidWidth / 2 + barWidth / 2 - triangleOffset
    return (
      <View>
        <View
          style={{
