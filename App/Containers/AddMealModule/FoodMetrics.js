...
export const MealTypes = {
  ...
  DINNER: 'DINNER',
  LATE_MEAL: 'LATE_MEAL',
  DRINK_FOOD_OTHER: 'DRINK_FOOD_OTHER'
}
export const MealTypeArray = [
  MealTypes.DRINK_FOOD_OTHER,
  MealTypes.LATE_MEAL,
  MealTypes.DINNER,
...
export const EatOutOptions = {
  TAKE_AWAY: 'EAT_OUT_OPTIONS.TAKE_AWAY',
  FAST_FOOD: 'EAT_OUT_OPTIONS.FAST_FOOD',
  RESTAURANT: 'EAT_OUT_OPTIONS.RESTAURANT',
  WORK_RESTAURANT: 'EAT_OUT_OPTIONS.WORK_RESTAURANT',
  FRIENDS: 'EAT_OUT_OPTIONS.FRIENDS',
  MISC: 'EAT_OUT_OPTIONS.MISC'
}
...
export const FoodUnits = {
  0: { unitNameDE: 'Gramm', unitId: 0 },
  1: { unitNameDE: 'Milliliter', unitId: 1 }
}
...
export function getPercentaceDelta (stage, food) {
  ...
  let percentageToAdd =
    percentPerUnit * gram * stage.stageFactor * stageCalculation.portionFactor
  // check if the result is a numeric value
  if (isNaN(percentageToAdd)) {
    log.warn(
      'Could not calculate percentage for',
      gram,
      'grams of',
      food.foodnameDE,
      '(Selected unit:',
      food.selectedAmount.value,
      food.selectedAmount.unit.unitNameDE,
      ')'
    )
    log.warn(
      'percentPerUnit: ' +
        ...
        stage.stageFactor +
        ' portionFactor: ' +
        stageCalculation.portionFactor
    )
    return 0
...
export function getBaseUnit (food) {
  if (typeof food.pyramidStages[0] !== 'undefined') {
    ...
    if (
      food.pyramidStages.length === 1 &&
      ['1.1', '1.2', '1.3', '2.6', '4.11', '6.30', '6.31a', '6.31'].includes(
        food.pyramidStages[0].fullStageName
      )
    ) {
      return 'ml'
    }
